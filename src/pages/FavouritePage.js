...
const FavouritePage = () => {
  ...
  // FETCH DATA
  const favDataFetch = async () => {
    setLoading(true);
    const response = await fetch(`${baseURL}/fetch-save-content`, {
      method: "GET",
      headers: {
  ...
  useEffect(() => {
    if (favData) {
      const result = favData.filter((data) =>
        data.content.title.toLocaleLowerCase().match(search.toLocaleLowerCase())
      );
      setData(result);
    }
  }, [search]);
  ...
  const columns = [
    {
      name: "Title",
      selector: (row) => row.content.title,
    },
    {
      name: "Price",
      selector: (row) => (row.content.price ? row.content.price : 0),
    },
    {
      name: "Author",
      selector: (row) => row.content.author,
    },
    {
      name: "",
      cell: (row) => (
        <>
          <Link className={styles.link} to={`/content/${row.content.id}`}>
            <GreyBtn>View</GreyBtn>
          </Link>
          {/* <div
  ...
  return (
    <>
      {isLoggedIn && !loading && favData && dashboardData && (
        <div className={styles.container}>
          <div className={styles.leftCon}>
            ...
            <div className={styles.optionsCon}>
              ...
              <Link className={styles.link} to="/dashboard/purchase">
                <p className={styles.option}>Purchase</p>
              </Link>

              <Link className={styles.link} to="/dashboard/favourites">
                <p className={styles.active}>Favourutes</p>
              </Link>
            </div>
          </div>
          ...
          <div className={classes.rightCon}>
            <div className={classes.rContainer}>
              <p className={classes.pTitle}>Favourites</p>
